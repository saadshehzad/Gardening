    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                return Response(
                    {"message": f"Error while creating article: {str(e)}"},
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR,
                )
        else:
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class UserPostLikeAPIView(APIView):
    ...
    def get(self, request, pk):
        ...
        user_post_likes = UserPostLike.objects.filter(user_post=user_post).values_list(
            "liked_by__username", flat=True
        )
    ...
    def delete(self, request, pk):
        ...
        try:
            user_post_like = UserPostLike.objects.get(
                user_post=user_post, liked_by=request.user
            )
            user_post_like.delete()
            return Response(
                {"detail": "You have removed the like successfully"},
                status=status.HTTP_204_NO_CONTENT,
            )
...
class UserPostCommentAPIView(APIView):
    ...
    def get(self, request, pk):
        ...
        comments_qs = UserPostComment.objects.filter(
            user_post=user_post
